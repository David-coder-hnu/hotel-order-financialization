...
A comparison with traditional hotel operations reveals meaningful cash-flow front-loading benefits: issuance revenue is concentrated at contract inception rather than spread over the operating period, converting uncertain future income into certain present capital and eliminating occupancy risk exposure for hotel operators.
...
\subsection{Economic Impact: Cash-Flow Front-Loading}

We compare the timing and certainty of hotel cash flows under traditional operations versus Time-Right issuance.
Rather than applying arbitrary cost multipliers, we use a direct discounted cash flow (DCF) comparison with a discount rate of 8\% (estimated WACC for the Chinese hotel industry) over an equivalent 36-month horizon.
Under the Time-Right model, hotels issue new Time-Rights every 12 months, receiving upfront cash in exchange for future room-night obligations.

\begin{table}[H]
\centering
\caption{Cash-flow comparison: Traditional vs.\ Time-Right model (per-hotel, per-year, normalized)}
\label{tab:comparison}
\begin{tabular}{@{}lrr@{}}
\toprule
\textbf{Metric} & \textbf{Traditional} & \textbf{Time-Right} \\
\midrule
Revenue timing       & Distributed over 36 months & Concentrated at issuance dates \\
Revenue certainty    & Subject to occupancy risk   & Locked in at issuance \\
Front-loading ratio  & 32.7\%                     & 33.3\% \\
\bottomrule
\end{tabular}
\end{table}

The economic benefit of the Time-Right model is not primarily a net-present-value gain---it is a \textbf{risk transfer and cash-flow timing improvement}.
By converting uncertain, back-loaded future revenue into certain, front-loaded present capital, hotels reduce their exposure to demand volatility and gain immediate capital for reinvestment.
The front-loading ratio improves modestly because the traditional model already generates relatively stable monthly cash flows; the Time-Right advantage lies in the \emph{certainty} of those cash flows, the elimination of occupancy risk, and the ability to raise capital without debt financing.

\textbf{Note on earlier results:} A previous version of this paper reported a 102\% NPV uplift, which was driven by hard-coded cost multipliers (operating cost rate 52\%, financing cost rate 20\%, risk cost rate 10\%) and benefit multipliers (working capital boost 30\%, risk transfer value 12\%). These multipliers lacked empirical calibration. The corrected DCF analysis removes all such multipliers, relying solely on engine-output cash flows and an explicit discount rate assumption with sensitivity analysis. The qualitative conclusion---that Time-Rights improve hotel cash-flow profiles through front-loading and risk transfer---is unchanged; only the magnitude of the benefit has been corrected.

\begin{figure}[H]
\centering
\includegraphics[width=0.7\textwidth]{figures/fig5_npv_comparison.png}
\caption{Cash-flow timing comparison: Traditional vs.\ Time-Right model. The Time-Right model concentrates cash inflows at issuance dates rather than spreading them over the operating period, converting uncertain future revenue into certain present capital.}
\label{fig:npv-comparison}
\end{figure}
