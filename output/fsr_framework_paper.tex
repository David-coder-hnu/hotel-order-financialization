...
\begin{figure}[H]
\centering
\includegraphics[width=0.75\textwidth]{figures/fig4_stress_test.png}
\caption{Stress test results: Senior tranche expected loss under progressively severe PD and LGD multipliers. The horizontal axis shows that even at 6$\times$ PD and 2$\times$ LGD---representing an extreme macroeconomic downturn---Senior EL remains below 0.01\%, confirming the robustness of the structural credit protection.}
...
\caption{Net present value comparison: Traditional hotel operations vs.\ Time-Right model. The Time-Right model front-loads future revenue through upfront issuance, generating a 102\% NPV uplift. The improvement is driven by the conversion of uncertain, discounted future cash flows into certain present capital.}
