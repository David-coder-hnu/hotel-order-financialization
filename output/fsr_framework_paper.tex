...
\begin{table}[H]
\centering
\caption{Cash-flow comparison: Traditional vs.\ Time-Right model (66-hotel pool, 8\% discount rate)}
\label{tab:comparison}
\begin{tabular}{@{}lrr@{}}
\toprule
\textbf{Metric} & \textbf{Traditional} & \textbf{Time-Right} \\
\midrule
3-year NPV (CNY)                        & 22.67B  & 3.62B   \\
Per-hotel per-year NPV (CNY)           & 114.5M  & 18.3M   \\
Issue revenue (CNY)                     & --      & 2.75B   \\
Redemption cost (CNY)                   & --      & 0.90B   \\
Hotel net benefit (CNY)                 & --      & 1.85B   \\
...
\textbf{Note on earlier results:} A previous version of this paper reported a 102\% NPV uplift, which was driven by two errors: (i) hard-coded cost and benefit multipliers without empirical calibration; and (ii) a unit conversion error---raw price data are recorded in \textit{fen} (1/100 CNY), which inflated traditional revenue figures by a factor of 100.
The corrected DCF analysis removes all multipliers, applies proper unit conversion, and models rolling issuance.
The qualitative conclusion---that Time-Rights improve hotel cash-flow profiles through front-loading and risk transfer---is unchanged; only the magnitude and sign of the NPV difference have been corrected.
