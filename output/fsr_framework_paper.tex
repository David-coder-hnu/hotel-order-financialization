...
A unified money-flow analysis reveals that Time-Rights generate \yen 1,227 of net system value per token. Hotels earn 56\% more annual revenue than OTA distribution (\yen 35.2M vs.\ \yen 22.6M per hotel-year), driven by the over-issuance multiplier ($\omega=1.29\times$) and occupancy-independent revenue certainty. Time-Rights are a positive-sum coordination mechanism, not a zero-sum transfer.
We model the hotel as an active secondary-market participant (adding $\approx$3.7\% of annual revenue) and analyze the over-issuance mechanism's fractional-reserve-like tail risk.
A stakeholder analysis reveals a positive-sum coordination mechanism creating \yen 1,227 of net value per Time-Right: hotels capture $\approx$67\% of system benefits, users 18\% (through market-driven booking discounts), the platform 8\%, and investors 7\%. Per available room-night, the Time-Right model delivers 55\% more hotel revenue than OTA distribution.
...
Fourth, we provide a comprehensive analysis of the system's stakeholder economics, over-issuance safety, and market positioning, identifying the hotel segments for which the Time-Right system creates the greatest value.
...
A complete money-flow analysis reveals a positive-sum coordination mechanism creating \yen 1,227 of net value per Time-Right.
Hotels capture 67\% of system benefits (\yen 817/TR) through guaranteed, front-loaded revenue that eliminates OTA commissions and occupancy risk.
Per available room-night, the Time-Right model delivers 55\% more revenue than OTA distribution (\yen 1,602 vs.\ \yen 1,032).
Users capture 18\% (\yen 221/TR) through market-driven booking discounts and price-lock benefits, funded by secondary-market liquidity rather than hotel margin compression.
The platform captures 8\% (\yen 98/TR) through the wholesale-retail spread and trading fees, with a deliberate 5$\times$ fee differential steering users toward secondary-market sales rather than cash redemption.
Investors capture 7\% (\yen 91/TR), earning $\approx$73\% nominal return for bearing hotel default risk and illiquidity.
The system's value is asymmetric across hotel tiers: economy hotels with high OTA dependence and costly financing benefit most; luxury hotels with stable occupancy benefit least.
...
The net result is that Time-Right issuance generates lower absolute NPV than traditional operations ($-42\%$ at baseline parameters; see Table \ref{tab:comparison}), but the cash received is \textbf{certain and immediate}---the hotel pays a discount for eliminating occupancy risk and gaining upfront liquidity.

\begin{table}[H]
\centering
\caption{Unified economic comparison: Traditional vs.\ Time-Right (66-hotel pool, reconciled with money-flow model)}
\label{tab:comparison}
\begin{tabular}{@{}lrr@{}}
\toprule
\textbf{Metric} & \textbf{Traditional (OTA)} & \textbf{Time-Right} \\
\midrule
Per-hotel annual revenue (CNY)           & 27.6M (gross) / 22.6M (net of OTA) & 35.2M (guaranteed) \\
Revenue certainty                        & Expected (62\% occupancy)           & Guaranteed (occupancy-independent) \\
OTA commission                           & 18\% of gross                       & 0\% \\
Per available room-night (CNY)          & 1,032 (expected)                    & 1,054 (guaranteed) \\
TR advantage per available room-night    & --                                  & +2\% per night, +56\% per hotel-year \\
System value per Time-Right              & --                                  & +\yen 1,227 (positive-sum) \\
\bottomrule
\end{tabular}
\end{table}
...
\textbf{Note on earlier results:} A previous version of this paper reported a 102\% NPV uplift, driven by three errors: (i) hard-coded cost and benefit multipliers without empirical calibration; (ii) a unit conversion error (raw data in \textit{fen}, 1/100 CNY); and (iii) triple-counting hotels in the traditional-mode computation. The corrected, reconciled analysis eliminates all three errors and derives all economic metrics from a single consistent money-flow model.
...
\midrule
\textbf{System Total} & \textbf{+1,227} & \textbf{100\%} & Positive-sum: \$1,227 value created per TR \\
\bottomrule
\end{tabular}
\end{table}
...
The key metric for hotel adoption is revenue per \emph{available} room-night: under the Time-Right model, the hotel earns \yen 1,602 per available room-night (guaranteed, occupancy-independent), compared to \yen 1,032 under OTA distribution (expected, at 62\% occupancy and 18\% commission).
This 55\% advantage is the system's primary value proposition for hotel operators.
...
The over-issuance mechanism creates a structural short position whose tail-risk properties are analyzed in Section 6.5.
...
The Time-Right model delivers a 55\% revenue increase per available room-night over OTA distribution (\yen 1,602 vs.\ \yen 1,032), because Time-Right revenue is guaranteed occupancy-independent while OTA revenue depends on realized occupancy.
